...
- 账号、密码保存在 iOS Keychain；查询缓存和诊断保存在 Scriptable 本地目录；
- 控制面板支持配置账号、立即更新、预览组件、清除本地数据；
...
账号和密码保存在当前设备的 iOS Keychain；查询缓存、诊断和登录会话状态保存在 Scriptable 本地目录。以上数据都不会写入 GitHub。查询引擎每次更新时从固定 Git 提交地址重新下载，不在设备上长期缓存。
